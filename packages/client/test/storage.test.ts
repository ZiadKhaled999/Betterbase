...
function createMockClient(responses: {
  ...
  const mockFetch = mock(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (url.includes('/upload') && init?.method === 'POST') {
      ...
      return new Response(JSON.stringify({ path: "test.txt", url: "https://example.com/test.txt" }), { status: 200 })
    }
    ...
    if (url.includes('/sign') && init?.method === 'POST') {
      return new Response(
        JSON.stringify({ signedUrl: responses.signedUrl ?? "https://example.com/signed.txt?sig=abc" }),
        { status: 200 }
      )
    }
    ...
    if (init?.method === 'DELETE') {
      return new Response(
        JSON.stringify({ message: responses.remove ?? "Files removed" }),
        { status: 200 }
      )
    }
    ...
    if (init?.method === 'GET') {
      return new Response(
        JSON.stringify(responses.list ?? { files: [] }),
        { status: 200 }
      )
    }
...
describe("StorageBucketClient", () => {
  describe("upload", () => {
    ...
    it("returns error when upload fails with non-ok response", async () => {
      const mockFetch = mock(async (): Promise<Response> => {
        return new Response(
          JSON.stringify({ message: "Upload failed - file too large" }),
          { status: 413 }
        )
      })
  ...
  describe("getPublicUrl", () => {
    ...
    it("returns error when getting public URL fails", async () => {
      const mockFetch = mock(async (): Promise<Response> => {
        return new Response(
          JSON.stringify({ message: "Bucket is private" }),
          { status: 403 }
        )
      })
  ...
  describe("remove", () => {
    it("removes single file successfully", async () => {
      const mockFetch = mock(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        ...
        return new Response(
          JSON.stringify({ message: "File removed successfully" }),
          { status: 200 }
        )
      })
    ...
    it("removes multiple files successfully", async () => {
      const mockFetch = mock(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        ...
        return new Response(
          JSON.stringify({ message: "3 files removed successfully" }),
          { status: 200 }
        )
      })
  ...
  describe("path encoding", () => {
    it("properly encodes special characters in file paths", async () => {
      const mockFetch = mock(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        ...
        return new Response(
          JSON.stringify({ path: "my folder/test&file.txt", url: "https://example.com/my folder/test&file.txt" }),
          { status: 200 }
        )
      })
