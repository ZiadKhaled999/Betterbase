...
export class StorageBucketClient {
		...
		try {
			...
			if (!response.ok) {
				...
				return {
					...
					error: {
						message:
							errorData.message || `Failed to create signed URL with status ${response.status}`,
						name: "SignedUrlError",
					},
				};
			}
	...
	async list(prefix?: string): Promise<BetterBaseResponse<StorageFile[]>> {
		try {
			const url = new URL(`${this.client.getUrl()}/api/storage/${encodeURIComponent(this.bucket)}`);
