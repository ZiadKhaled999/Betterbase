...
export function exportSDL(
	...
	const { includeDescriptions = true, useCommentSyntax = true, sortTypes = false } = options;

	// Helper to convert Maybe<string> to string | undefined
	const toStringOrUndefined = (val: string | null | undefined): string | undefined =>
		val ?? undefined;

	const lines: string[] = [];

	// Helper to format descriptions
	const formatDescription = (description: string | null | undefined, indent = ""): string => {
	...
	const formatType = (type: unknown, indent = ""): string => {
	...
	if (scalarTypes.length > 0) {
		...
		for (const scalar of scalarTypes) {
			lines.push(formatDescription(toStringOrUndefined(scalar.description)));
	...
	if (enumTypes.length > 0) {
		...
		for (const enumType of enumTypes) {
			lines.push(formatDescription(toStringOrUndefined(enumType.description)));
			lines.push(`enum ${enumType.name} {`);
			for (const value of enumType.getValues()) {
				lines.push(formatDescription(toStringOrUndefined(value.description), "  "));
	...
	if (interfaceTypes.length > 0) {
		...
		for (const interfaceType of interfaceTypes) {
			lines.push(formatDescription(toStringOrUndefined(interfaceType.description)));
			const interfaces = interfaceType.getInterfaces();
			const interfaceStr =
				interfaces.length > 0 ? ` implements ${interfaces.map((i) => i.name).join(" & ")}` : "";
	...
	if (unionTypes.length > 0) {
		...
		for (const unionType of unionTypes) {
			lines.push(formatDescription(toStringOrUndefined(unionType.description)));
	...
	if (inputTypes.length > 0) {
		...
		for (const inputType of inputTypes) {
			lines.push(formatDescription(toStringOrUndefined(inputType.description)));
	...
	if (regularObjectTypes.length > 0) {
		...
		for (const objectType of regularObjectTypes) {
			lines.push(formatDescription(toStringOrUndefined(objectType.description)));
	...
	if (queryType) {
		...
		lines.push(formatDescription(toStringOrUndefined(queryType.description)));
	...
	if (mutationType) {
		...
		lines.push(formatDescription(toStringOrUndefined(mutationType.description)));
	...
	if (subscriptionType) {
		...
		lines.push(formatDescription(toStringOrUndefined(subscriptionType.description)));
...
export function exportTypeSDL(
	...
	if (type instanceof GraphQLObjectType || type instanceof GraphQLInputObjectType) {
		...
		lines.push(formatDescription(toStringOrUndefined(type.description)));
