...
const createMockProvider = (supportsRLS: boolean, supportsGraphQL: boolean = true): ProviderAdapter => {
	return {
		type: "neon",
		dialect: "postgres",
...
const createMockDbConnection = (executeFn?: () => void): DatabaseConnection => {
	const mockDrizzle = {
		execute: executeFn ? vi.fn().mockImplementation(executeFn) : vi.fn().mockResolvedValue({ rows: [] }),
	}
	return {
...
describe("migration/index", () => {
	describe("runMigration", () => {
		...
		it("applies policies when RLS is supported", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				expect.stringContaining("Applying RLS policies"),
			)
