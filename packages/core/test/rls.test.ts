...
describe("rls/generator", () => {
	...
	describe("policiesToSQL", () => {
		it("generates SQL for multiple policies", () => {
			...
			expect(sql.some((s) => s.includes("ALTER TABLE users ENABLE ROW LEVEL SECURITY;"))).toBe(
				true,
			);
			expect(sql.some((s) => s.includes("ALTER TABLE posts ENABLE ROW LEVEL SECURITY;"))).toBe(
				true,
			);
		});
	});
