 ...
 */
async function runFunctionCreate(
 ...
 */
async function runFunctionBuild(
  name: string | undefined,
  projectRoot: string
): Promise<void> {
  if (!name) {
 ...
 * Show function logs
 */
async function runFunctionLogs(
  name: string | undefined,
  projectRoot: string
): Promise<void> {
  if (!name) {
...
async function runFunctionDeploy(
  ...
  if (syncEnv && config && config.env.length > 0) {
    ...
    if (runtime === 'cloudflare-workers') {
      const syncResult = await syncEnvToCloudflare(
        name,
        config,
        projectRoot,
        envValues
