...
export class RealtimeServer {
  ...
  authorize(userId: string, claims: string[], table: string): boolean {
    return Boolean(userId) && (claims.includes('realtime:*') || claims.includes(`realtime:${table}`));
  }
  ...
  handleMessage(ws: ServerWebSocket<unknown>, rawMessage: string): void {
    ...
    if (!result.success) {
      this.safeSend(ws, { error: 'Invalid message format', details: result.error.format() });
      return;
    }
