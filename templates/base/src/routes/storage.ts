...
import { createStorage, type StorageFactory } from '@betterbase/core/storage';
...
function sanitizePath(path: string): string {
  ...
  if (sanitized.includes('..') || sanitized.startsWith('/')) {
    throw new HTTPException(400, { message: 'Invalid path: path traversal not allowed' });
  }
...
storageRouter.delete('/:bucket', async (c) => {
  try {
    ...
    return c.json({ message: result.data?.message || 'Files deleted successfully' });
  } catch (error) {
    if (error instanceof HTTPException) {
      throw error;
    }
